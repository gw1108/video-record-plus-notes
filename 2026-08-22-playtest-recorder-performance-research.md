...
- **Owner: the required native helper process (§4.3, tech-stack report §3.6)** — Electron cannot capture renderer-free, and OBS's standard output would composite the webcam into the game video rather than keep it as a separate track.
...
- **Separate files vs multi-track**: mic-in-OBS-track adds zero container overhead; the webcam as a separate small file adds one more sequential writer — trivially within budget. Cap.so ships separate screen+webcam tracks in production as validation of the pattern ([Cap](https://cap.so/open-source-screen-recorder)).
...
│   │   / Device) → NVENC single-pass CQP,                  │
│   │   AQ/lookahead OFF, canvas=output, NV12               │
│   ├─ **mic = audio track 2 (same file — sync free)**      │
│   └─ Hybrid MP4 on internal SSD ≠ game drive              │
│                                                           │
│  **Native helper (required): webcam track — MJPEG         │
│  passthrough file OR 2nd NVENC session — plus Raw         │
│  Input hotkey fallback (not Electron getUserMedia)**      │
└───────────────────────────────────────────────────────────┘
              ▼ stop
