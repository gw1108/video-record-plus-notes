...
_seen: 2 · home: .claude/docs/agent-tooling.md_
Even a quoted heredoc (`cat > f <<'EOF'`) halves a doubled backslash on the way to disk, so a JS literal written with an escaped backslash before `$Recycle.Bin` lands with a bare one — which JS then reads as the drive-relative `C:$Recycle.Bin`, and the failure surfaces as a confusing `ENOENT ... scandir 'C:\<cwd>\$Recycle.Bin'`. This corrupts silently: the script parses and runs, it just points somewhere else. Use forward slashes in Windows paths (Node accepts them), or write the file with the Write tool, which does no escaping. Verify what actually landed with `grep -n <pattern> <file> | cat -A`, not by re-reading your own heredoc. The same trap has a second face: an inline `python - <<'EOF'` patch script whose *non-raw* literal carries a Windows path — the heredoc halves the doubled backslash, so `\\r` in `<dir>\\report` reaches Python as a real carriage return and splits that line in the file it writes, while `\\p` only raises a SyntaxWarning and survives. Use raw strings or `chr(92)` for backslashes in those literals, and check the result with `repr()` on the text read back with `newline=""` rather than trusting the printed diff. A large heredoc can also fail to reach bash at all (`unexpected EOF while looking for matching quote` on content with no unbalanced quote), so switch to the Write tool as soon as a heredoc misbehaves instead of re-shaping the quoting.
...
### An offscreen Electron window's capturePage hands back the previous frame
_seen: 1 · home: .claude/docs/electron-harnesses.md_
A harness window created with `show: false` composites lazily, so `webContents.capturePage()` called right after a DOM change screenshots the state before it — the evidence PNG shows the upload still at 73% on the shot meant to prove the success panel, while the DOM assertions on that same state pass. Sleep a few hundred ms inside the screenshot helper (or show the window) before capturing, and never treat a screenshot as the assertion: assert on `executeJavaScript` reads, and keep the image as evidence for a human.
